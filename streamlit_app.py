...
with col2:
    ...
    st.bar_chart(pd.DataFrame.from_dict(counts, orient="index", columns=["shots"]))
...
for r in range(0, 7):
    ...
    counts_r = run_counts(qc_r, shots=shots)
    rows.append({"iterations": r, "success_probability": success_probability(counts_r, int(target), n)})
